...
import { X, Dumbbell, Video, Wrench } from 'lucide-react';
                ...
                <div className="p-8">
                    ...
                    <div className="flex flex-wrap gap-2 mb-6">
                        ...
                        {exercise.muscleGroups.map((muscle, idx) => (
                            <span
                                key={idx}
                                className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-slate-800/60 text-slate-300 rounded-lg border border-white/5"
                            >
                                <Dumbbell className="w-3.5 h-3.5" /> {muscle}
                            </span>
                        ))}
                    </div>
                    ...
                    {exercise.videoUrl && (
                        <div className="mb-6">
                            <h3 className="text-lg font-semibold text-white mb-3 flex items-center gap-2"><Video className="w-5 h-5 text-lime-400" /> How to Perform</h3>
                    ...
                    {exercise.muscleDiagramUrl && (
                        <div className="mb-6">
                            <h3 className="text-lg font-semibold text-white mb-3 flex items-center gap-2"><Dumbbell className="w-5 h-5 text-lime-400" /> Target Muscles</h3>
                    ...
                    <div className="mb-6">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {exercise.equipment.map((item, idx) => (
                                <span
                                    key={idx}
                                    className="inline-flex items-center gap-1.5 px-3 py-2 bg-slate-800/60 text-slate-300 rounded-lg border border-white/5"
                                >
                                    <Wrench className="w-3.5 h-3.5" /> {item}
                                </span>
                            ))}
                        </div>
                    </div>
