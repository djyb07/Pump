/**
 * SmartFilterBar Component - Unified Sticky Filter Toolbar
 *
 * A reusable, responsive filter bar with glassmorphism styling.
 * Uses sticky positioning so it flows naturally below the page title,
 * then sticks to the top when the user scrolls past it.
 *
 * On desktop, it accounts for the SmartNavbar (h-16) by adjusting
 * its sticky top offset based on navbar visibility.
 *
 * Features:
 * - Sticky position: sits below title, sticks on scroll
...
export function SmartFilterBar({
    ...
    return (
        <div
            className={`sticky z-40 w-full border-b border-white/5
                bg-slate-900/30 backdrop-blur-xl
                transition-[top] duration-300 ease-in-out
                top-0 ${isVisible ? 'md:top-16' : 'md:top-0'}`}
        >
